...
export default function useView(viewId){
  ...
  console.log('useView render', { viewId, value, error })
  ...
  useEffect(
    () => {
      ...
      const eventSource = new EventSource(`/api/views/${viewId}?subId=${subId}`)
      console.log({ subId, eventSource })
      eventSource.onopen = (...args) => {
        console.log('useView open', subId, args)
      }
      eventSource.onerror = (error) => {
        console.log('useView error', subId, error)
        setError(error)
      }
      eventSource.onmessage = (event) => {
        console.log('useView onMessage', subId, event, event.data)
        const value = JSON.parse(event.data)
        console.log('useView onMessage', subId, value)
        setValue(value)
      }
      return () => {
        console.log('useView close', subId)
        eventSource.close()
      }
    },
    [viewId]
