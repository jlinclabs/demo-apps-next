...
import { useMyIdentifiers, useCreateIdentifier } from '../../lib/identifierHooks'
import Layout from '../../components/Layout'
import Link from '../../components/Link'
import ErrorMessage from '../../components/ErrorMessage'
import Timestamp from '../../components/Timestamp'

export default function IdenitifiersPage({ router }) {
  const createIdentifier = useCreateIdentifier({
    onSuccess(idenitifier){
      console.log('CREATED', { idenitifier })
      router.push(`/identifiers/${idenitifier.did}`)
    },
    onFailure(error){
      console.error(error)
    },
...
function MyIdentifier({ identifier }){
  return <ListItem {...{
    ...
    <ListItemButton {...{
      ...
      dense: true,
      component: Link,
      href: `/identifiers/${identifier.did}`,
